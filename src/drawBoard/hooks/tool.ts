...
import { Point, Shape, ToolInterface } from '../types'
...
export function useTool(
  ...
  { onMove, temp, persist, interfaceLayer, history, brush }: UseShapeProps
) {
  const { lazy, brushColor, brushRadius } = brush
  ...
  function onTouchEnd(e: GestureResponderEvent) {
    e.preventDefault()
    if (!isPressing.current || !isDrawing.current) return
    ...
    const result = onSave(persist.ctx.current, shape, { brush })
    if (result !== false) {
      const namedShape =
