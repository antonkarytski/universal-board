import {BrushOptions, Point, ToolInterface} from '../types'
...
export function createBaseTool({
  ...
  draw,
  drawInterface,
}: CreateBaseShapeProps): ToolInterface {
  return {
    name,
    isLazyAvailable: false,
    onDrawMove(ctx, points, {width, height, interfaceLayer, ...brushSettings}) {
...
// export function createBaseToolClass({
//   name,
//   draw,
//   drawInterface,
// }: CreateBaseShapeProps) {
//   return class {
//     name = name
//     isLazyAvailable = false
...
//     }
//
//     onRepeat: RepeatAction = (ctx, {points, ...brushSettings}) => {
//       if (!ctx || points.length < 2) return
//       setBrushSettings(ctx, brushSettings)
//       draw(ctx, points)
//     }
//     onSave: SaveAction = (ctx, {points, ...brushSettings}) => {
//       if (!ctx || points.length < 2) return false
//       setBrushSettings(ctx, brushSettings)
//       draw(ctx, points)
//     }
...
export function dummyShape(name: string): ToolInterface {
  return {
    name,
    isLazyAvailable: false,
    onDrawMove() {},
